
<div class="article content-team-hydro template-team-hydro" id="team">
  <div class="row">
    <div class="team-hydro-titre">

      <h1>TEAM <span class="hydrocontest-text">{{$equipePrincipale->nom}}</span> <span class="heigvd-text"> HEIG-VD</span></h1>

    </div>
    <div class="team-description">
      <div class="row team">

          <div class="col-md-4 template-membre team-member">
            <img src="{{asset('img/johnaesch.jpg')}}" class="img-responsive image-membre" id="img-membre">
            <div class="membre-description">
              <div class="text">
                <h5 id="name">nom prenom</h5>
                <p id="fonction">adieu salut</p>
                <p id="description">"wcwiucw cwhbweibwec ew debiw"</p>
              </div>
            </div>
          </div>


      </div>
    </div>
  </div>
</div>
